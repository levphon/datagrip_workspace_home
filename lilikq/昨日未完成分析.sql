...
#汇总
select userId, any_value(username) as username, sum(cnt)
from (select tcp.user_id userId, tu.username, count(lcluefollowup.id) cnt
      from (select tc.id, count(tcf.id) fu_cnt
            from t_clue tc
                     left join t_clue_followup tcf on tcf.clue_id = tc.id and tcf.del_flag = 0
            where tc.del_flag = 0
      ...
      where tut.del_flag = 0
        and tut.status = 0
        and date(tut.notify_time) < curdate()
      group by tut.created_by) tall
where userId in (select user_id from t_assignment_user where del_flag = 0)
group by userId;